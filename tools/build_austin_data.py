...
def build_profile():
    ...
    prof = {
        "bins": bins,
        # veh/h entering (15-min counts x4)
        "nbVeh": [round(v * 4) for v in series(nb_feed, 0)],
        ...
        "samplesPerBin": {
            "nb": round(sum(v[2] for v in nb_feed.values()) / max(1, len(nb_feed))),
        ...
        "meta": {
            ...
            "nbSensor": "Lamar & Collier NB_in (S Lamar feed)",
            "sbSensor": "Lamar & Sandra Muraida SB_in (bridge approach)",
    ...
    print(f"NB peak {peak_nb} veh/h at {bins[peak_i]}; "
          f"SB peak {max(prof['sbVeh'])} veh/h at {bins[prof['sbVeh'].index(max(prof['sbVeh']))]}",
          file=sys.stderr)
